# noise (从'general'类别中随机选取噪音)
# 特点：背景噪音与目标语音频谱相似，且每次处理的噪音文件都可能不同

SCENE_CONFIG = {
    "scene_name": "noise",  # 可以改个名字以示区别
    "metadata": {
    ...
    "effects": [
        ...
        },
        {
            "name": "change_volume",
            "params": {
                "target_lufs": {"random_type": "uniform", "min": -25, "max": -15, "is_core": True}  # 音量降低10到30分贝，模拟距离衰减
            }
        },
        {
            "name": "add_noise",
            "params": {
                ...
                # 噪声只比信号低 10dB，非常嘈杂
                "noise_db": {"random_type": "uniform", "min": -15, "max": -5, "is_core": True}
            }
        },
        # {
        #     "name": "adjust_speed",
